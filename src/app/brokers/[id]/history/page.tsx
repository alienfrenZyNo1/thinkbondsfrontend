...
export default function BrokerHistoryPage({ params }: { params: { id: string } }) {
  ...
  // Check if user has access to view broker history
  const canViewBrokerHistory = groups.includes(UserRole.ADMIN) ||
                              groups.includes(UserRole.WHOLESALE) ||
                              groups.includes(UserRole.AGENT) ||
                              groups.includes(UserRole.BROKER);

  useEffect(() => {
    if (!canViewBrokerHistory) {
      return;
    }
    ...
    const fetchEditHistory = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/brokers/${params.id}/history`);
        if (!response.ok) {
    ...
    fetchEditHistory();
  }, [params.id, canViewBrokerHistory]);

  if (!canViewBrokerHistory) {
    return (
      <ProtectedRoute>
        <div className="p-6">
          <h1 className="text-3xl font-bold mb-4">Broker Edit History</h1>
          <div className="bg-yellow-50 border border-yellow-200 rounded p-4">
            <p className="text-yellow-700">You don't have permission to view this broker's edit history.</p>
          </div>
        </div>
      </ProtectedRoute>
    );
    ...
    <ProtectedRoute>
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold">Broker Edit History</h1>
          <button
            onClick={() => router.back()}
        ...
        {!loading && !error && (
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            {editHistory.length === 0 ? (
              <p>No edit history found for this broker.</p>
            ) : (
