...
export default function BondHistoryPage({
  ...
  useEffect(() => {
    ...
    const fetchEditHistory = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/bonds/${params.id}/history`);
        if (!response.ok) {
    ...
    <ProtectedRoute>
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold">Bond Edit History</h1>
          <button
            onClick={() => router.back()}
        ...
        {!loading && !error && (
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            {editHistory.length === 0 ? (
              <p>No edit history found for this bond.</p>
            ) : (
