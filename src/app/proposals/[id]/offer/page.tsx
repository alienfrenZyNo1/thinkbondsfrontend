...
import { generateSecureToken, generateTimeLimitedToken, generateOTP, hashValue } from "@/lib/security";
...
export default function CreateOfferPage({ params }: { params: { id: string } }) {
 ...
 const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  // Fetch proposal data
  const { data: proposal, isLoading: isProposalLoading, error: proposalError } = useQuery({
  ...
  const { data: policyholder, isLoading: isPolicyholderLoading, error: policyholderError } = useQuery({
  ...
  // Fetch broker data
  const { data: broker, isLoading: isBrokerLoading, error: brokerError } = useQuery({
      ...
      <ProtectedRoute>
        <div className="p-6">
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded p-4">
            <p className="text-yellow-700">You don't have permission to create offers.</p>
          </div>
        </div>
      </ProtectedRoute>
    );
  ...
  const handleOfferSubmit = async (values: any) => {
    ...
    try {
      ...
      const token = generateTimeLimitedToken(tokenData, 86400); // 24 hours expiry
        ...
        {(proposalError || policyholderError || brokerError) && (
          <div className="bg-red-50 border border-red-200 rounded p-4 mb-6">
            <p className="text-red-700">
              Error loading data: {proposalError?.message || policyholderError?.message || brokerError?.message}
            </p>
          </div>
        )}
        ...
        {success && (
          <div className="bg-green-50 border border-green-200 rounded p-4 mb-6">
            <p className="text-green-700">
              Offer created successfully! The policyholder and beneficiary will receive an email with acceptance instructions.
            </p>
            <div className="mt-4">
              <Button onClick={() => router.push(`/proposals/${params.id}`)}>Back to Proposal</Button>
            </div>
          </div>
        )}
