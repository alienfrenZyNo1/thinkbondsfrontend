...
export const handlers = [
  ...
  http.get('https://keycloak.example.com/realms/thinkbonds/protocol/openid-connect/auth', async ({ request }) => {
  ...
  http.post('https://keycloak.example.com/realms/thinkbonds/protocol/openid-connect/token', async () => {
    // Add delay to simulate network request
    await new Promise(resolve => setTimeout(resolve, 300));
  ...
  http.get('https://keycloak.example.com/realms/thinkbonds/protocol/openid-connect/userinfo', async () => {
    // Add delay to simulate network request
    ...
    if (sessionTokenMatch && sessionTokenMatch[1]) {
      try {
        ...
        if (tokenParts.length === 5) {
          ...
          if (sessionTokenMatch[1].includes('broker')) {
            userRole = 'broker';
          ...
          userRole = tokenData.user?.groups?.[0] || tokenData.dominoData?.groups?.[0] || 'admin';
          console.log('Session endpoint - Decoded JWT token:', JSON.stringify(tokenData, null, 2));
        }
  ...
  http.post('/api/users', async ({ request }) => {
    ...
    // Add delay to simulate network request
    await new Promise(resolve => setTimeout(resolve, 300));
    return HttpResponse.json({ id: 4, ...newUser as any }, { status: 201 })
  }),
  ...
  http.post('/api/registration/verify-pin', async ({ request }) => {
    const body = await request.json() as any
  ...
  http.post('/api/registration/submit', async ({ request }) => {
    // Add delay to simulate network request
    await new Promise(resolve => setTimeout(resolve, 500));
 ...
 // Creditsafe reports handler
  ...
  http.get('/api/bonds/:id', async ({ params }) => {
    ...
    return HttpResponse.json({
      ...
      terms: "This bond is issued subject to the terms and conditions outlined in the agreement.",
      status: "active",
      createdAt: new Date().toISOString(),
    })
  }),
  ...
  http.get('/api/bonds/:id/history', async ({ params }) => {
    const id = params.id as string;
  ...
  http.post('/api/bonds/:id/reject', async ({ params, request }) => {
    const id = params.id as string;
    const body = await request.json() as any;
 ...
 http.post('/api/bonds/:id/validate-otp', async ({ params, request }) => {
  ...
  http.get('/api/offers/bin', async () => {
    ...
    const softDeletedOffers = mockOffers.filter((offer: any) => offer.status === 'soft_deleted');
    
    return HttpResponse.json(softDeletedOffers)
  }),
  
  // Offer history handler
  http.get('/api/offers/:id/history', async ({ params }) => {
    const id = params.id as string;
  ...
  http.get('/api/brokers/bin', async () => {
    ...
    const softDeletedBrokers = mockBrokers.filter((broker: any) => broker.status === 'soft_deleted');
    
    return HttpResponse.json(softDeletedBrokers)
  }),
  
  http.get('/api/brokers/:id/history', async ({ params }) => {
    const id = params.id as string;
  ...
  http.get('/api/policyholders/bin', async () => {
    ...
    const softDeletedPolicyholders = mockPolicyholders.filter((policyholder: any) => policyholder.status === 'soft_deleted');
    
    return HttpResponse.json(softDeletedPolicyholders)
  }),
  
  http.get('/api/policyholders/:id/history', async ({ params }) => {
    const id = params.id as string;
  ...
  http.get('/api/proposals/bin', async () => {
    ...
    const softDeletedProposals = mockProposals.filter((proposal: any) => proposal.status === 'soft_deleted');
    
    return HttpResponse.json(softDeletedProposals)
  }),
  
  http.get('/api/proposals/:id/history', async ({ params }) => {
    const id = params.id as string;
